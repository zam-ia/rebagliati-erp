...
export default function TabPlanilla() {
  ...
  const guardarNovedad = async () => {
    ...
    const registro = {
      ...
      justificacion: form.justificada ? (form.justificacion || 'Justificado') : null
    };

    const { error } = await supabase.from('asistencia').insert([registro]);
  ...
  const resumen = novedades.reduce((acc, n) => {
    ...
    else if (n.justificacion) acc.justificadas++;
    else if (n.falta) acc.faltas++;
    return acc;
  }, { tardanzas: 0, justificadas: 0, faltas: 0 });
  ...
  return (
    <div className="p-1">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => setModal({})} className="bg-[#185FA5] text-white px-5 py-2 rounded-xl text-sm font-bold shadow-lg hover:scale-105 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-red-50 border border-red-100 p-5 rounded-2xl">
          ...
          <div className="text-[10px] font-black text-red-400 uppercase tracking-widest">Tardanzas Injust.</div>
        </div>
        <div className="bg-amber-50 border border-amber-100 p-5 rounded-2xl">
          <div className="text-4xl font-black text-amber-700">{resumen.justificadas}</div>
          <div className="text-[10px] font-black text-amber-400 uppercase tracking-widest">Justificadas</div>
        </div>
        <div className="bg-gray-50 border border-gray-100 p-5 rounded-2xl">
          <div className="text-4xl font-black text-gray-700">{resumen.faltas}</div>
          <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Faltas Totales</div>
        </div>
      </div>
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl w-full max-w-md p-8">
            <h2 className="text-2xl font-bold mb-6 italic text-[#185FA5]">Registrar Incidencia</h2>
            <div className="space-y-4">
              ...
              {form.tipo === 'Tardanza' && (
                <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                  ...
                  <input type="number" value={form.minutos_tardanza} onChange={e => setForm({...form, minutos_tardanza: e.target.value})} className="w-full bg-transparent text-xl font-bold outline-none" />
                </div>
              )}

              <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                <input type="checkbox" checked={form.justificada} onChange={e => setForm({...form, justificada: e.target.checked})} className="w-5 h-5" id="checkModal" />
                <label htmlFor="checkModal" className="font-bold text-gray-600">¿Justificada?</label>
              </div>

              <div className="flex gap-4 pt-4">
                <button onClick={guardarNovedad} className="flex-1 bg-[#185FA5] text-white py-4 rounded-2xl font-bold shadow-lg shadow-blue-200">Guardar</button>
